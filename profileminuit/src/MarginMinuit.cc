...
MarginMinuit* MarginMinuit::gGlobalMarginMinuitInstance = NULL;

MarginMinuit::MarginMinuit() {
  ...
  SetMinuitFCN( &DefaultMarginMinuitFCN );
}
...
void MarginMinuit::Initialize( int numbins, ParameterManager* parMan ) {
  if ( GetVerbose()>=1 ) {
    std::cout << "---------------------------------" << std::endl;
    std::cout << " MarginMinuit::Initialize " << std::endl;
  }
  ...
  if ( fNbins<0 ) {
    ...
    std::cout << " Somewhere, somehow, MarginMinuit::SetNbins( n ) must be called" << std::endl;
    std::cout << "---------------------------------" << std::endl;
  ...
  if ( GetVerbose()>=1 ) {
    Print();
    std::cout << " End of MarginMinuit::Initialize " << std::endl;
    std::cout << "---------------------------------" << std::endl;
    if ( GetVerbose()>=1 ) {
...
void MarginMinuit::SetFitterInterface( MarginMinuitI* fitter_interface ) {
  // Override Fitter:SetFitterInterface( ... ) so that can type-check interface
  MarginMinuitI* userInterface = dynamic_cast< MarginMinuitI* >( fitter_interface );
  if ( fitter_interface && userInterface==NULL ) {
    std::cout << "------------------------------------------" << std::endl;
    std::cout << "MarginMinuit::SetFitterIterface" << std::endl;
    std::cout << " The interface class passed to the fitter, " << fitter_interface << ", does not inherit from MarginMinuitI." << std::endl;
    std::cout << "------------------------------------------" << std::endl;
    std::exit(EXIT_FAILURE);
  }
...
void MarginMinuit::IndexParameters() {
  GetParManager()->Initialize();
  fParsIndexed = true;
}

void MarginMinuit::InitializeMinuit() {
...
void MarginMinuit::SetMinuitVerbose( int verbose ) {
...
void MarginMinuit::InitializePullMinimizer() {
...
void MarginMinuit::PushParStatusToPullMinimizer() {
...
void MarginMinuit::FloatPar( std::string name ) {
 if ( IsParameterDefined( name ) ) {
    ...
    if ( par->GetFitterParType()==kMinuitTerm && fMinuitInitialized) {
      std::cout << "MarginMinuit::FloatPar(" << name << ") - Releasing parameter (MinuitIndex=" << GetParManager()->GetMinuitIndex(name) << ")" << std::endl;
...
void MarginMinuit::FixPar( std::string name ) {
  if ( IsParameterDefined( name ) ) {
    ModelParameter* par = GetParameter(name);
    par->FixParameter();
...
void MarginMinuit::FloatAllPars() {
  if ( fMinuitInitialized && GetNMinuitTerms()>0) {
    std::vector< std::string > minuitpars;
    GetParManager()->GetListOfMinuitParNames( minuitpars );
    for ( std::vector< std::string >::iterator it=minuitpars.begin(); it!=minuitpars.end(); it++ ) {
...
void MarginMinuit::SetParameterValue( std::string name, double value ) {
...
void MarginMinuit::GetParametersFromMinuit() {
  if ( fMinuitInitialized && GetNMinuitTerms()>0) {
    if ( GetVerbose()>=2 )
...
void MarginMinuit::GetParametersFromPullMinimizer() {
...
void MarginMinuit::PushParametersToMinuit() {
  std::vector< std::string > minuitpars;
  GetParManager()->GetListOfMinuitParNames( minuitpars );
  for ( std::vector< std::string >::iterator it=minuitpars.begin(); it!=minuitpars.end(); it++ ) {
...
void MarginMinuit::PushParametersToPullMinimizer() {
  for ( ParameterManager::ParListIter it=GetParManager()->ParListBegin(); it!=GetParManager()->ParListEnd(); it++ ) {
    std::string parname = *it;
    ModelParameter* par = GetParManager()->GetParameter(parname);
...
void MarginMinuit::AddFitResultBranchesToTree( TTree* tree ) {
...
void MarginMinuit::PrintNewtonFitterInfo() {
...
//   UserLoadFijValues( GetParManager() ); /// Finally, User must calculated Fij values
  ...
  if ( GetVerbose()>=1 ) {
    Print();
    std::cout << "End of MarginMinuit::RunFit, status=";
...
bool MarginMinuit::RunMinuit() {
  ...
  if ( GetVerbose()>=2 ) 
    std::cout << "MarginMinuit::RunMinuit [ max calls=" << miparms[0] << ", tolerance=" << miparms[1] << " ]" << std::endl;
...
bool MarginMinuit::RunPullMinimizer() {
  SetNewtonSolverVerbose( fNewtonVerbose );
  
  if ( GetNNewtonTerms() == 0 || GetNumVariedNewtonTerms()==0 ) {
    if ( GetVerbose()>=2 ) std::cout << "MarginMinuit::RunPullMinimizer: Skipped because zero Newton parameters" << std::endl;
    fLastMinimizerCallConverged = true;
    return true;
  }
  
  if ( GetVerbose()>=2 )
    std::cout << "MarginMinuit::RunPullMinimizer" << std::endl;
  ...
  while ( m_tries<maxtries && minimizerConverged==false) {
    if ( GetVerbose()>=2 ) {
      std::cout << "MarginMinuit::RunPullMinimizer(). Newton Minimizer called. Attempt " << m_tries << " of " << maxtries << std::endl;      
    }
    ...
    if ( GetVerbose()>=2 )  {
      std::cout << "MarginMinuit::RunPullMinimizer()  Fit returned with Chi-Squared=" << ChiSquared << " (chi-error=" << chierror << ") status=";
  ...
  if ( GetVerbose()>=2 ) {
    std::cout << "End of MarginMinuit::RunPullMinimizer. Resulting parameter values:" << std::endl;
...
double MarginMinuit::GetChiSquared() {
...
double MarginMinuit::GetPullChiSquared() {
  // Must use all parameters now, not just newton parameters
  // However, the indexing doesn't work for this yet
  int fNTerms = GetNMinuitTerms()+GetNNewtonTerms();
  double pullValues[ fNTerms ];
...
void MarginMinuit::SetBinLabel( int binnum, std::string name ) {
  m_pull_min->SetBinLabel( binnum, name );
}

void MarginMinuit::SetBinNExpected( int binnum, double expected, double* gradient, int ngradpars ) {
  m_pull_min->SetBinNExpected( binnum, expected );
  if ( gradient!=NULL && ngradpars!=GetParManager()->NumberOfParameters() ) {
    std::cout << "WARNING - MarginMinuit::SetBinExpected(...) was given " << std::endl;
...
void MarginMinuit::SetBinNObserved( int binnum, double observed ) {
  m_pull_min->SetBinNObserved( binnum, observed );
}

void MarginMinuit::SetFij( std::string pullName, int binNum, double fij ) {
  if ( !IsParameterDefined( pullName ) ) assert(false);
  m_pull_min->SetFij( GetParManager()->GetParameterIndex( pullName ), binNum, fij );
}

void MarginMinuit::SetFij( int parIndex, int binNum, double fij ) {
  m_pull_min->SetFij( parIndex, binNum, fij );
}

void MarginMinuit::PrintFij() {
  m_pull_min->PrintFij();
}

void MarginMinuit::SetPullCentralValue( std::string parname, double minimum ) {
  if ( !IsParameterDefined( parname ) ) {
    std::cout << "MarginMinuit::SetPullCentralValue - '" << parname << "' has not yet been defined." << std::endl;
    assert(false);
    exit(EXIT_FAILURE);
  }

  if ( !m_pull_min || !fPullMinimizerInitialized ) {
    std::cout << "MarginMinuit::SetPullCentralValue - the Pull Minimizer has not yet been initilized yet." << std::endl;
    assert(false);
    exit( EXIT_FAILURE );
  }
...
void MarginMinuit::SendGradientToMinuit( bool doit ) {
  fGiveMinuitGradient = doit;
}

void MarginMinuit::BuildGradientArray( int nbins, int npars ) {
  m_grad_nbins = nbins;
  m_grad_npars = npars;
...
void MarginMinuit::DestroyGradientArray() {
  delete [] bin_gradients;
  bin_gradients = NULL;
}

double MarginMinuit::GetGradientElement( int ibin, int ipar ) {
  return *(bin_gradients + m_grad_npars*ibin + ipar );
}

void MarginMinuit::SetGradientElement( int ibin, int ipar, double grad ) {
  *(bin_gradients + m_grad_npars*ibin + ipar ) = grad;
}

double* MarginMinuit::GetBinGradient( int ibin ) {
  return ( bin_gradients + m_grad_npars*ibin );
}

double MarginMinuit::GetChiSquaredPartial( int parid ) {
...
void MarginMinuit::UpdateExpectation() {
...
void MarginMinuit::UserLoadExpectationBins( ParameterManager* parameters ) {
  std::cout << " ------------------------------------------------------------------------------------------ " << std::endl;
...
void MarginMinuit::UserLoadObservedBins( ParameterManager* parameters ) {
  std::cout << " ------------------------------------------------------------------------------------------ " << std::endl;
...
void MarginMinuit::UserLoadFijValues( ParameterManager* parameters ) {
...
double MarginMinuit::UserReturn1SigmaShiftInBinByNewtonParameter( int binNum, std::string parname, ModelParameter* par ) {
  std::cout << " ------------------------------------------------------------------------------------------ " << std::endl;
...
void DefaultMarginMinuitFCN( int &ndim, double* grad_optional, double &result, double par[], int flag ) {
  MarginMinuit* mm = MarginMinuit::GetGlobalInstance();


  if ( mm->GetVerbose() ) {
    std::cout << "=========================================================================================================" << std::endl;
    std::cout << " DefaultMarginMinuitFCN" << std::endl;
    std::cout << "---------------------------------------------------------------------------------------------------------" << std::endl;
  }
  ...
  if ( mm->GetVerbose()>=1 ) {
    std::cout << "------------------------------------" << std::endl;
    std::cout << "DefaultMarginMinuitFunction[ flag=" << flag << "]" << std::endl;
    ...
    std::cout << "---------------------------------------------------------------------------------------------------------" << std::endl;
    std::cout << " End of DefaultMarginMinuitFCN" << std::endl;
    std::cout << "=========================================================================================================" << std::endl;
